import { Button, Popover, DatePicker, useBreakpoints, Icon, InlineGrid, Box, Select, Scrollable, OptionList, BlockStack, InlineStack, TextField } from '@shopify/polaris';
import {
  CalendarIcon,
  ArrowRightIcon,
...
export default function DateRangePicker() {
  ...
  const [{ month, year }, setDate] = useState<YearMonth>({
  ...
  function isNodeWithinPopover(node: Node): boolean {
  ...
  function handleInputBlur({ relatedTarget }: React.FocusEvent<HTMLInputElement>): void {
  ...
  useEffect(() => {
    if (activeDateRange) {
      ...
      function monthDiff(
        ...
        newDate: { year: number; month: number }
      ): number {
        return (
          newDate.month -
          referenceDate.month +
            ...
            {mdDown ? (
              <Select
                ...
                onChange={(value) => {
                  ...
                  setActiveDateRange(result ?? ranges[0]);
                }}
